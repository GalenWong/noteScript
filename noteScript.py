...
def grapher(paragraphs, p_salience):
	...
	def putBrackets(noun, nounPhrase):
		start = nounPhrase.find(noun)+len(noun)
		return ("(" + noun + ")" + nounPhrase[end:])

	def findjustNouns():
		jNouns = []
		for sentence in salience:
			subject = list(sentence.keys())
			jNouns.extend(subject)
		temp = []
		for word in jNouns:
			if word not in temp:
				temp.append(word)
	...
	def insertNode (word):
		for sentence in para:
			...
			if word in subjectPhrase:
				removeAll(subjectPhrase, nouns)

				#Isolate first noun from subjectPhrase
				tokenized_words = word_tokenize(subjectPhrase)
				match = False
				subject = subjectPhrase
				for token in tokenized_words:
					for possible_match in justNouns:
						if token == possible_match:
							match = True
							subject = possible_match
							break
					if match:
						break	
				...
				for verb in verbs:
					...
					text.add_node(subject,traversed=False)
					text.add_node(obj,traversed=False)
					text.add_edge(subject, obj, weightage = 0, label = verb)
					if  obj != "":
						insertNode(obj)
					removeAll(obj, nouns)
		...
		for child in text.neighbors(headNode):
			...
			if  text.edges[headNode, child]['weightage'] == 1:
				if text.nodes[child]['traversed'] != True:
					str1 = (tabs + " - " + child + " " + text.edges[headNode, child]['label'] + " it/them" + "\n")
					output.write("%s" % str1)
				...
				if text.nodes[child]['traversed'] != True:
					...
					str2 = (tabs + " - " + text.edges[headNode, child]['label'] + " ")
					text.nodes[child]['traversed'] = True
					output.write("%s" % str2)
					removeAllNodes(child,allNodes)
